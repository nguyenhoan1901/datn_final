		...
		<div class="table-responsive">
			<table class="table" id="datatable">
				...
				@foreach($commander as $user)
					<tr>
						<td>{{$user->id}}</td>
						<td><a href="{{route('commander.edit', $user->id)}}">{{$user->username}}</td>
						<td>{{$user->name}}</td>
						<td>{{$user->email}}</td>
						{{-- <td>@if($user->is_admin==1)Yes @else No @endif</td> --}}
                        <td>
                            <div class="list-icons">
                                <a href="{{route('commander.edit', $user->id)}}" class="list-icons-item text-primary-600"><i class="icon-pencil7"></i></a>
