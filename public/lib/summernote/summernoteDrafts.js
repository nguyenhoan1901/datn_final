  ...
  (function(factory) {
    ...
    $.extend($.summernote.lang['en-US'], {
      sDrafts: {
        save: 'Lưu nháp',
        load: 'Tải nháp',
        select: 'Chọn bản nháp đã lưu',
        provideName: 'Provide a name for this draft',
        saved: 'Nháp đã được lưu',
        loaded: 'Đã tải dữ liệu nháp',
    ...
    return $.extend($.summernote.plugins, {
      'sDraftsLoad': function(context) {
        ...
        fn = function() {
          ...
          return htmlList += "<li class='list-group-item'><a href='#' class='note-draft' data-draft='" + key + "'>" + draft.name + " - <small>" + fDate + "</small></a><a href='javascript:;' class='label label-danger pull-right delete-draft' data-draft='" + key + "'>" + lang.deleteDraft + "</a></li>";
        };
        for (key in drafts) {
          draft = drafts[key];
          fn();
